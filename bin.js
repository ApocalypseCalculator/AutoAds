...
if (!fs.existsSync('./data.json')) {
    console.log(chalk.yellow('Missing data.json file, recreating one'));
    fs.writeFileSync('./data.json', JSON.stringify({chrome: "", email: ""}));
}
if(!fs.existsSync('./selector.bat')) {
    console.log(chalk.yellow('Missing selector.bat file, recreating one'));
    fs.writeFileSync('./selector.bat', `@echo off
